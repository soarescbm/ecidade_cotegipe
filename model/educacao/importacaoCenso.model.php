...
abstract class importacaoCenso {
   ...
   * @param string  $sCaminhoArquivo - Se for null ele não filtra pela escola, usado na importação
   * de alunos para clientes novos.
   * @param integer $iAnoEscolhido
   * @param integer $iCodigoInepEscola
   */
  function __construct($sCaminhoArquivo, $iAnoEscolhido, $iCodigoInepEscola = null, $iCodigoLayout) {
    ...
    if ($oDaoSecParametros->numrows > 0) {
      $oDadosSecParametros = db_utils::fieldsmemory($rsSecParametros, 0);
    }
   ...
   * @param integer $iCodCensoCartorio
   * @param string  $sNomeCartorio
   * @param integer $iServentia
   * @return integer ed291_i_codigo
   */
  function getCartorio($iCodigoCensoCartorio = null, $sNomeCartorio = null, $iServentia = null) {
    ...
    } elseif ($sNomeCartorio != null) {
      $sWhere = "ed291_c_nome = '".$sNomeCartorio."'";
    } elseif( $iServentia != null ) {
   ...
   */
  function getCensoMunicipioCertidao($iCodCartorio) {
  ...
  function getMatriculasRechumano($oLinha) {
    ...
    } elseif (isset($oLinha->codigodocenteescola) && !empty($oLinha->codigodocenteescola)) {
 ...
 */
function getDadosEscola($oLinha) {
 ...
 * @param object $oLinha com os campos contidos em uma linha de importacao (conforme seu tipo de registro)
 */
function atualizaDadosEscola($oLinha) {
 ...
 * @param object $oLinha com os campos contidos em uma linha de importacao (conforme seu tipo de registro)
 */
function atualizaDadosEscolaEstrutura($oLinha) {
  ...
  if ($oDadosEscola != null) {
    ...
    if ($oDaoEscolaEstrutura->numrows > 0) {
      ...
      if ($oDaoEscolaEstrutura->erro_status == '0') {
        ...
        throw new Exception("Erro na alteração dos dados da infra estrutura da escola. Erro da classe: ".
                            $oDaoEscolaEstrutura->erro_msg
                           );
 ...
 * @param object $oLinha com os campos contidos em uma linha de importacao (conforme seu tipo de registro)
 */
function atualizaDadosTurma($oLinha) {
...
function atualizaDadosDocente($oLinha) {
   ...
   * @param object $oLinha com os campos contidos em uma linha de importacao (conforme seu tipo de registro)
   */
  function atualizaEnderecoDocente($oLinha) {
  ...
  * @param object $oLinha com os campos contidos em uma linha de importacao (conforme seu tipo de registro)
   */
  function atualizaEscolaridadeDocente($oLinha) {
    ...
    if ($aDadosRechumano != null) {
      ...
      for ($iCont = 0; $iCont < $iTam; $iCont++) {
        ...
        if (isset($oLinha->situacaocurso1) && isset($oLinha->formacao1) && isset($oLinha->cursosuperior1)
          ...
          $aFormacao[0] = array(trim($oLinha->situacaocurso1),
        ...
        if (isset($oLinha->situacaocurso2) && isset($oLinha->formacao2) && isset($oLinha->cursosuperior2)
          ...
          $aFormacao[1] = array(trim($oLinha->situacaocurso2),
        ...
        if (isset($oLinha->situacaocurso3) && isset($oLinha->formacao3) && isset($oLinha->cursosuperior3)
          ...
          $aFormacao[2] = array(trim($oLinha->situacaocurso3),
   ...
   * @param object $oLinha com os campos contidos em uma linha de importacao (conforme seu tipo de registro)
   */
  function atualizaDadosAluno($oLinha) {
    ...
    if (trim($oLinha->racaaluno) != "") {
      ...
      } elseif ($oLinha->racaaluno == 1) {
        $oDaoAluno->ed47_c_raca = 'BRANCA';
      } elseif ($oLinha->racaaluno == 2) {
        $oDaoAluno->ed47_c_raca = 'PRETA';
      } elseif ($oLinha->racaaluno == 3) {
        $oDaoAluno->ed47_c_raca = 'PARDA';
      } elseif ($oLinha->racaaluno == 4) {
   ...
   * @param object $oLinha com os campos contidos em uma linha de importacao (conforme seu tipo de registro)
   */
   function atualizaEnderecoDocumentosAluno($oLinha) {
    ...
    if ($aDadosAluno != null) {
      ...
      for ($iCont = 0; $iCont < $iTam; $iCont++) {
        ...
        if (!empty($oLinha->tipodecertidao)
          ...
          } elseif ($oLinha->tipodecertidao == 2) {
   ...
   * @param object $oLinha com os campos contidos em uma linha de importacao (conforme seu tipo de registro)
   */
  function atualizaDadosEscolarizacaoAluno($oLinha) {
  ...
  public function validarImportacao() {
    ...
    if (!$this->pArquivoLog) {
      throw new Exception($sMsgErro . " Não foi possível abrir o arquivo de log! ");
    }
