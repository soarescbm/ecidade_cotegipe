...
class RelatorioDisponibilidadeFinanceira {
  ...
  private function getContaCorrenteDetalhe($iContaCorrente) {
    ...
    $sSqlContaCorrenteDetalhe = $oDaoContaCorrenteDetalhe->sql_query_fileAtributos(null, $sCampos, $sOrder, $sWhere);
   ...
   * @throws DBException
   */
  private function getLancamentosContaCorrenteDetalhe($iContaCorrente, $iInstituicao, $iReduzido, $iRecurso, $iCaracteristicaPeculiar = null) {
  ...
  private function buscaDadosImplantacao($aReduzidos, $aRecursos, $iCaracteristicaPeculiar = null) {
    ...
    $sSql = $oDaoContaCorreteSaldo->sql_query_busca_saldo_implantacao($sCampos, $sWhere, $this->oDataInicial->getAno());
  ...
  private function getTotalLancamentos() {
    ...
    if (empty($this->aContasCorrenteBuscar) || !is_array($this->aContasCorrenteBuscar)) {
      throw new ParameterException("Atributo iContaCorrente informado inválido.");
    }
    ...
    $oDaoContaCorrenteDetalheLancamento = new cl_contacorrentedetalheconlancamval();
    $sSqlLancamentos = $oDaoContaCorrenteDetalheLancamento->sql_query_lancamentos($sCampos, $sWhere . $sGroupBy, $sOrderBy);
    $rsLancamentos   = db_query($sSqlLancamentos);
    if (!$rsLancamentos) {
  ...
  private function getDados() {
    ...
    foreach ($oContaCorrente->aContaCorrenteDetalhe as $oContaCorrenteDetalhe) {
      ...
      $oSaldosIniciais         = $this->getSaldosIniciais($oContaCorrenteDetalhe->c19_contacorrente,
                                                          array($oContaCorrenteDetalhe->c19_reduz),
                                                          array($oContaCorrenteDetalhe->c19_orctiporec),
      ...
      if ($this->iMostrarLancamentos == self::MOSTRAR_LANCAMENTOS_SIM) {
        ...
        $oContaCorrenteDetalhe->aLancamentos = $this->getLancamentosContaCorrenteDetalhe($oContaCorrenteDetalhe->c19_contacorrente,
                                                                                         $oContaCorrenteDetalhe->c19_instit,
                                                                                         $oContaCorrenteDetalhe->c19_reduz,
                                                                                         $oContaCorrenteDetalhe->c19_orctiporec,
                                                                                         $iCaracteristicaPeculiar);
      }

      $oTotalLancamentos = $this->getTotalLancamentosContaCorrenteDetalhe($aTotalLancamentosContaCorrenteDetalhe, $oContaCorrenteDetalhe);
  ...
  private function escreverSaldoConta(stdClass $oContaCorrente) {
    ...
    $this->oPdf->setBold(false);
    $this->oPdf->Cell($this->iLarguraPagina * 0.2,
                      $this->iAlturaLinha,
                      $oContaCorrente->sSaldoInicialContaDebito,
                      'TB',
                      0,
  ...
  private function escreverCabecalhoRecurso(stdClass $oDado) {
    ...
    $this->oPdf->Cell($this->iLarguraPagina * 0.8,
                      $this->iAlturaLinha,
                      "{$oDado->c19_orctiporec} - {$oDado->o15_descr}",
                      0,
                      1,
                      'L');
    ...
    $this->oPdf->Cell($this->iLarguraPagina * 0.8,
                      $this->iAlturaLinha,
                      "{$oDado->c19_reduz} - " . db_formatar($oDado->c60_estrut, 'receita')
                      . " -  {$oDado->c60_descr} ",
                      0,
                      1,
                      'L');
  ...
  private function escreverSaldoInicialRecurso(stdClass $oDado) {
    ...
    $this->oPdf->setBold(true);
    $this->oPdf->Cell($this->iLarguraPagina * 0.4, $this->iAlturaLinha);
    $this->oPdf->Cell($this->iLarguraPagina * 0.2, $this->iAlturaLinha, "SALDO INICIAL DO RECURSO:", $sBorda, 0, "R",$iPreenchimento);
    $this->oPdf->setBold(false);
    $this->oPdf->Cell($this->iLarguraPagina * 0.2, $this->iAlturaLinha, $oDado->sSaldoInicialRecursoDebito, $sBorda, 0, "R", $iPreenchimento);
    $this->oPdf->Cell($this->iLarguraPagina * 0.2, $this->iAlturaLinha, $oDado->sSaldoInicialRecursoCredito, $sBorda, 1, "R", $iPreenchimento);
  }
  ...
  private function escreverTotalizadoresRecurso(stdClass $oDado) {
    ...
    $this->oPdf->Cell($this->iLarguraPagina * 0.2,
                      $this->iAlturaLinha,
                      db_formatar($oDado->nTotalLancamentosDebito, 'f'),
                      'TB',
                      0,
                      'R',
                      1);
    $this->oPdf->Cell($this->iLarguraPagina * 0.2,
                      $this->iAlturaLinha,
                      db_formatar($oDado->nTotalLancamentosCredito, 'f'),
                      'TB',
                      1,
                      'R',
                      1);
    ...
    $this->oPdf->setBold(false);
    $this->oPdf->Cell($this->iLarguraPagina * 0.2, $this->iAlturaLinha, $oDado->sSaldoFinalRecursoDebito, 'TB', 0, 'R', 1);
    $this->oPdf->Cell($this->iLarguraPagina * 0.2, $this->iAlturaLinha, $oDado->sSaldoFinalRecursoCredito, 'TB', 1, 'R', 1);
  ...
  private function escreverLinhaLancamento(stdClass $oLancamento) {
    ...
    $this->oPdf->Cell($this->iLarguraPagina * 0.1,
                      $this->iAlturaLinha,
                      db_formatar($oLancamento->c71_data, 'd'),
                      0,
                      0,
                      'C');
    $this->oPdf->Cell($this->iLarguraPagina * 0.15, $this->iAlturaLinha, $oLancamento->c69_codlan, 0, 0, 'R');
    $this->oPdf->Cell($this->iLarguraPagina * 0.35,
                      $this->iAlturaLinha,
                      $oLancamento->c53_coddoc . " " . $oLancamento->c53_descr,
                      0,
                      0,
                      'L');
    $this->oPdf->Cell($this->iLarguraPagina * 0.2,
                      $this->iAlturaLinha,
                      db_formatar($oLancamento->nValorDebito, 'f'),
                      0,
                      0,
                      'R');
    $this->oPdf->Cell($this->iLarguraPagina * 0.2,
                      $this->iAlturaLinha,
                      db_formatar($oLancamento->nValorCredito, 'f'),
                      0,
                      1,
                      'R');
  }
  ...
  private function escreverTotalizadoresConta(stdClass $oContaCorrente) {
    ...
    $this->oPdf->setBold(true);
    $this->oPdf->Cell($this->iLarguraPagina * 0.25,
                      $this->iAlturaLinha,
                      "TOTAIS DA MOVIMENTAÇÃO DA CONTA:",
                      'TB',
                      0,
                      'R',
                      1);
    ...
    $this->oPdf->Cell($this->iLarguraPagina * 0.2,
                      $this->iAlturaLinha,
                      db_formatar($oContaCorrente->nTotalMovimentacaoDebito, 'f'),
                      'TB',
                      0,
                      'R',
                      1);
    $this->oPdf->Cell($this->iLarguraPagina * 0.2,
                      $this->iAlturaLinha,
                      db_formatar($oContaCorrente->nTotalMovimentacaoCredito, 'f'),
                      'TB',
                      1,
                      'R',
                      1);
  ...
  private function getTotalLancamentosContaCorrenteDetalhe($aTotalLancamentos, stdClass $oContaCorrenteDetalhe) {
    ...
    } else if (isset($aTotalLancamentos[$iContaCorrente][$iReduzido][$iInstituicao][$iRecurso][$iCaracPeculiar])) {
