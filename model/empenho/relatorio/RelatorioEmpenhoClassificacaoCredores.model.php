...
class RelatorioEmpenhoClassificacaoCredores {
  ...
  public function emitir() {
    ...
    foreach ($aInformacoes as $iIndice => $oStdInformacao) {
      ...
      if ($iUltimaClassificaoImpressa != $oStdInformacao->codigo_classificacao) {
        ...
        if ($this->exibirAPagar()) {
          $this->imprimirTotalizador("TOTAL A PAGAR:",
                                     $aTotalizadoresPorLista[$iUltimaClassificaoImpressa]->total_pagar,
                                     $this->aPagar());
        }
      ...
      $this->oPdf->cell(87,
                        4,
                        substr($oStdInformacao->razao_social, 0, 95),
                        0,
                        0,
                        PDFDocument::ALIGN_LEFT,
                        $lPreencher);
      $this->oPdf->cell(30, 4, db_formatar($oStdInformacao->valor, 'f'), 0, 1, PDFDocument::ALIGN_RIGHT, $lPreencher);

      $iPaginaAntesJustificativa = $this->oPdf->getCurrentPage();
      if (!empty($oStdInformacao->justificativa_pagamento)) {

        $this->oPdf->setBold(true);
        $this->oPdf->cell($this->oPdf->getAvailWidth(),
                          4,
        ...
        $this->oPdf->MultiCell($this->oPdf->getAvailWidth(),
                               4,
                               $oStdInformacao->justificativa_pagamento,
                               0,
                               PDFDocument::ALIGN_JUSTIFY,
                               $lPreencher);
      }
      ...
      if ($oListaClassificacao->dispensa() && !empty($oStdInformacao->justificativa_dispensa)) {
        ...
        $this->oPdf->MultiCell($this->oPdf->getAvailWidth(),
                               4,
                               $oStdInformacao->justificativa_dispensa,
                               0,
                               PDFDocument::ALIGN_JUSTIFY,
                               $lPreencher);
        $this->oPdf->ln(3);
  ...
  private function imprimeCabecalhoLista($iCodigo) {
    ...
    $this->oPdf->cell($this->oPdf->getAvailWidth(), 4, "Lista de Classificação de Credores: {$sClassificacao}", 'B', 1);
    $this->oPdf->setBold(false);
    $this->oPdf->SetFontSize(6);
  }
  ...
  private function getDadosImprimir() {
    ...
    $aCampos = array(
      "e60_numemp  as codigo_empenho", "e60_codemp  as numero_empenho", "e60_anousu  as ano",
      "e69_codnota as codigo_nota", "e69_numero  as numero_nota", "e50_codord  as codigo_ordem",
      "z01_nome    as razao_social", "e69_dtvencimento as data_vencimento", "e69_dtrecebe as data_recebimento",
      "o58_codigo as codigo_recurso", "e53_valor  as ordem_valor_total", "e53_vlranu as ordem_valor_anulado",
      'e09_sequencial', 'cc30_codigo as codigo_classificacao', 'corempagemov.k12_data as data_pagamento',
      'sum(case when k12_valor is null then e81_valor else k12_valor end) as valor',
      'trim(e09_justificativa) as justificativa_pagamento', 'trim(cc31_justificativa) as justificativa_dispensa'
    );

    $aWhere = array(
      "e60_instit = {$this->oInstituicao->getCodigo()}", 'e81_cancelado is null',
