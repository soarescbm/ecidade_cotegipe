...
class LicitanteLicitaCon extends ArquivoLicitaCon {
  ...
  private function getAutoresEvento(stdClass $oDadosLicitacao) {
    ...
    $sSqlEvento    = $oDaoEvento->sql_query_file(null, "l46_cgm as autor", null, "l46_liclicita = {$oDadosLicitacao->CODIGO_LICITACAO} and l46_cgm is not null");
    ...
    for ($iRowAutor = 0; $iRowAutor < $iTotalRegistros; $iRowAutor++) {
      ...
      $oStdDadosRetorno->TP_RESULTADO_HABILITACAO = 'I'; // Se não é fornecedor, então manda sempre inabilitado
  ...
  private function prepararLicitantesVencedores(array $aCampos) {
    ...
    $aWhere = array(
      ...
      "l44_sigla in ('CNS', 'PRE', 'LEI', 'PRP')",
      "l44_sigla not in ('PRD', 'PRI', 'RPO')",
      "l17_situacao = 1"
    );

    $aWherePadrao = LicitacaoLicitaCon::getWhereLicitacao($this->oCabecalho->getInstituicao(), $this->oCabecalho->getDataGeracao());
    $aWhere = array_merge($aWhere, $aWherePadrao);

    $oDaoLicitacao = new cl_liclicita();
    $sSqlBuscaLicitacoes = $oDaoLicitacao->sql_query_licitantes("distinct ".implode(',', $aCampos), implode(' and ', $aWhere));
  ...
  private function prepararLicitantesGerais(array $aCampos) {
    ...
    $aWhere = array(
      "l44_sigla not in ('CNS', 'PRE', 'LEI', 'PRP')",
      "l44_sigla not in ('PRD', 'PRI', 'RPO')",
      "l17_situacao = 1"
    );

    $aWhere = array_merge($aWhere, LicitacaoLicitaCon::getWhereLicitacao($this->oCabecalho->getInstituicao(), $this->oCabecalho->getDataGeracao()));
  ...
  private function criarObjetoImpressao(stdClass $oStdBaseDados) {
    ...
    $oStdLicitantes->TP_RESULTADO_HABILITACAO = self::getTipoHabilitacaoFornecedor($oStdBaseDados->fornecedor_licitacao);
    $oStdLicitantes->BL_BENEFICIO_MICRO_EPP   = $this->oRegra->getBeneficioMicroEpp($oLicitacao, $oStdBaseDados->tipo_empresa);
  ...
  private static function getTipoCondicaoFornecedor($iCodigoFornecedor) {
    ...
    if (pg_num_rows($rsBuscaCondicao) > 0) {
      $iCodigoSituacao = db_utils::fieldsMemory($rsBuscaCondicao, 0)->pc31_tipocondicao;
    }
    return !empty($iCodigoSituacao) ? $aValores[$iCodigoSituacao] : '';
  }
   ...
   * @throws DBException
   */
  private static function getTipoHabilitacaoFornecedor($iCodigoFornecedor) {

    $oDaoHabilitacao = new cl_pcorcamfornelichabilitacao();
    $sSqlHabilitacao = $oDaoHabilitacao->sql_query_file(null, 'l17_situacao', null, "l17_pcorcamfornelic = {$iCodigoFornecedor}");
    $rsBuscaHabilitacao = db_query($sSqlHabilitacao);
    if (!$rsBuscaHabilitacao) {
      throw new DBException("Não foi possível buscar o tipo de habilitação do fornecedor.");
    }
    $aValores = array(1 => 'H', 2 => 'I', 3 => 'N');
    return pg_num_rows($rsBuscaHabilitacao) > 0 ? $aValores[db_utils::fieldsMemory($rsBuscaHabilitacao, 0)->l17_situacao] : '';
  }
