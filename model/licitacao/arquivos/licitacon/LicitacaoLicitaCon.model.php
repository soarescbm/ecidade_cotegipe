...
class LicitacaoLicitaCon extends ArquivoLicitaCon {
  ...
  public function getDados() {
    ...
    for ($iLicitacao = 0; $iLicitacao < $iTotalLicitacoes; $iLicitacao++) {
      ...
      $sNumneroProcesso = filter_var($aProcesso[0], FILTER_SANITIZE_NUMBER_INT);
      ...
      $oDados->NR_PROCESSO             = !empty($oStdLicitacao->p58_numero) ? $oStdLicitacao->p58_numero : $sNumneroProcesso;
      $oDados->ANO_PROCESSO            = !empty($oStdLicitacao->p58_ano)    ? $oStdLicitacao->p58_ano    : $iAnoProcesso;
      $oDados->TP_NIVEL_JULGAMENTO     = $this->oRegra->getTipoJulgamentoSigla();
      $oDados->TP_NATUREZA             = $this->getTipoNatureza($oLicitacao);
      $oDados->TP_RESULTADO_GLOBAL     = $this->oRegra->getResultadoGlobal();
      $oDados->DS_OBJETO               = $oStdLicitacao->l20_objeto;
      $oDados->VL_LICITACAO            = $this->getValorLicitacao($oLicitacao);
      ...
      $oDados->BL_ORCAMENTO_SIGILOSO   = null;
      ...
      if ((in_array($oDados->CD_TIPO_MODALIDADE, array('RDC')) && in_array($oDados->TP_LICITACAO, array('MPR', 'TPR', 'MOP')))
        || (in_array($oDados->CD_TIPO_MODALIDADE, array('PRP', 'PRE')) && in_array($oDados->TP_LICITACAO, array('MPR')))) {
        $oDados->BL_ORCAMENTO_SIGILOSO = 'N';
      }

      if (in_array($oDados->CD_TIPO_MODALIDADE, array('CHP', 'CNC', 'CNV', 'CPC', 'PRE', 'PRP', 'RDC', 'RIN', 'TMP'))
  ...
  /**
   * Aplica a regra para o tipo de natureza, conforme manual do Licitacon.
   * @param licitacao $oLicitacao
   *
   * @return null|string
   */
  private function getTipoNatureza(licitacao $oLicitacao) {

    /**
     * Visto com Peres que qualquer licitação que usa Registro de Preço deve retornar R
     */
    if ( $oLicitacao->usaRegistroDePreco() ) {
      return 'R';
    }
    ...
    if (in_array($sModalidade, array('CNV', 'TMP', 'LEI', 'CON', 'PRD', 'PRI'))) {
      return 'N';
    }

    if (in_array($sModalidade, array('CHP', 'CPC', 'RIN', 'MAI'))) {
      return 'O';
    }
  ...
  /**
   * Retorna o valor total da licitação, conforme regra do manual do LicitaCon.
   * @param licitacao $oLicitacao
   *
   * @return string
   * @throws DBException
   */
  private function getValorLicitacao(licitacao $oLicitacao) {
  ...
  /**
   * Preenche o oobjeto $oDados com as informações referentes aos atributos donâmicos de cada Licitação.
    ...
    foreach ($aValoresAtributosDinamicos as $oValor) {
      ...
      switch ($oValor->getAtributo()->getNome()) {
        ...
        case "tipobeneficiomicroepp":
          $oDados->TP_BENEFICIO_MICRO_EPP = $sValor;
          break;
        ...
        case "pctaxarisco":
          ...
          if (Check::isFloat($sValor)) {

            $nValor = filter_var($sValor, FILTER_VALIDATE_FLOAT);
            $oDados->PC_TAXA_RISCO = number_format($nValor, 2, ',', '');
          }
          break;
        ...
        case "tipodisputa":
          $oDados->TP_DISPUTA = $sValor;
          break;

        case "prequalificacao":
          $oDados->TP_PREQUALIFICACAO = $sValor;
          break;
       ...
       */
      if (!empty($sModalidade) && $sModalidade == 'RPO') {
