...
class Menu extends Model {
  ...
  public function getItensMenu($iInstitId, $iAreaId, $iModuloId, $iItemMenuId = null) {
    ...
    if ( !$this->sOrdenacaoMenu ) {
      ...
      $this->sOrdenacaoMenu = ( $oPreferencias->getOrdenacao() == "alfabetico" ? "descricao" : "menusequencia" );
    }
  ...
  public function getBreadcrumbMenu($id) {
    ...
    $breadcrumb = null;
    $id = (int) $id;
    $result = $this->db->execute("select fc_montamenu($id) as breadcrumb");

    if ($result && pg_num_rows($result) > 0) {
      $breadcrumb = $this->db->fetchRow($result, 0)->breadcrumb;
    }
