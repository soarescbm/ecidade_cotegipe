...
<script>
  ...
  function js_salvar() {
    ...
    $$('input:checkbox:checked:enabled').forEach(function (element) {
      aSelecionados.push({'iCompetencia': element.value});
    });
    ...
    oParametros.gerarAssentamentos = parseInt($F('iGerarAssentamentos'));

    var oAjaxRequest = new AjaxRequest(sUrl, oParametros,
      function (oAjax, lErro) {

        alert(oAjax.message.replace(/\\n/g, '\n'));
    ...
    oAjaxRequest.setMessage("Processando encerramento, esta operação pode demorar alguns minutos.");
    oAjaxRequest.execute();
  }
</script>