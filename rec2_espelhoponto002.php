...
try {
  ...
  $aPeriodos          = $oPeriodoRepository->getPeriodosEntreDatas(new DBDate($oParametros->sDataInicio), new DBDate($oParametros->sDataFim));
  ...
  foreach ($aServidores as $servidor) {
    ...
    foreach ($aDatasServidor as $indDatas => $oData) {
      ...
      if (!$oData->oJornada->dsr_folga || $oData->lTemMarcacoes) {
        ...
        $oData->entrada1  = montarMarcacao($oEntrada1, $lEmiteTodosAfastamentos, $oData->afastamento, $lMostrarLegendaAfastamentos);
        $oData->saida1    = montarMarcacao($oSaida1, $lEmiteTodosAfastamentos,$oData->afastamento, $lMostrarLegendaAfastamentos);
        $oData->entrada2  = montarMarcacao($oEntrada2, $lEmiteTodosAfastamentos,$oData->afastamento, $lMostrarLegendaAfastamentos);
        $oData->saida2    = montarMarcacao($oSaida2, $lEmiteTodosAfastamentos,$oData->afastamento, $lMostrarLegendaAfastamentos);
        $oData->entrada3  = montarMarcacao($oEntrada3, $lEmiteTodosAfastamentos,$oData->afastamento, $lMostrarLegendaAfastamentos);
...
function escreverGrade (PDF $pdf, $dados, $lHeader = false) {
  ...
  $colunas = (array)$dados;
  $iMaximoDeLinhas   = 5;
  $aColunasNaoContar = array('afastamento', 'oJornada', 'data_dia', 'aMarcacoes', 'oPeriodoEfetividade', 'data');
