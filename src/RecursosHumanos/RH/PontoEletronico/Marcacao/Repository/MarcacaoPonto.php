...
class MarcacaoPonto {
  ...
  public function getCollectionMarcacaoPonto(DiaTrabalho $oDiaTrabalho) {
    ...
    $aMarcacoes = \db_utils::makeCollectionFromRecord($rsMarcacao, function($oRetorno) use ($oMarcacaoPontoRepository, $aHoras) {
  ...
  private function getMarcacoesEntrePeriodos(DiaTrabalho $oDiaTrabalho) {
    ...
    if(!empty($aHoras)) {
      ...
      $oDataInicio->setTime($aHoras[0]->oHora->format('H'), $aHoras[0]->oHora->format('i'));
      $oDataFim->setTime($oUltimaHora->oHora->format('H'), $oUltimaHora->oHora->format('i'));
    }
    ...
    if($oDataInicio->format('Y-m-d') == $oDataFim->format('Y-m-d')) {
      $oDataFim->modify("+1 day");
    }
    ...
    $aCampos = array('rh197_data', 'rh198_registro', 'rh198_sequencial', 'rh198_registro_manual::int', 'rh198_ordem', 'rh198_data');
  ...
  public function getJustificativasPorMarcacao(MarcacaoPontoModel $oMarcacaoPonto) {
    ...
    return \db_utils::makeFromRecord($rsJustificativa, function($oRetorno) use($oMarcacaoPonto) {
