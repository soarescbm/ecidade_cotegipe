...
class Alteracao extends BaseAbstract {
	...
	public function getValores(\stdClass $oPosicaoAtual, \stdClass $oPosicaoAnterior = null) {
		...
		if (!is_null($nValorAcrescimo) && $nValorAcrescimo < 0) {
			$nValorAcrescimo = $nPcAcrescimo = null;
		}
		...
		$oValores->valor_acrescimo      = $nValorAcrescimo < 0 ? null : $nValorAcrescimo;
		$oValores->valor_reducao        = $nValorReducao < 0 ? null : $nValorReducao;
		$oValores->percentual_acrescimo = $nPcAcrescimo < 0 ? null : $nPcAcrescimo;
		$oValores->percentual_reducao   = $nPcReducao < 0 ? null : $nPcReducao;
   ...
   */
    ...
    return empty($nValor) ? '0,00' : number_format($nValor, 2, ',', '');
  }
