...
class EmissaoRecibos
{
  ...
  /**
   * @param boolean $lProcessaMassaFalida - Se deve processar as matriculas que estão cadastradas como massa falida
   * @param boolean $lSomenteEnderecoEntregaValido - Se deve processar somente matriculas com endereço de entrega válido
   * @param boolean $lGerarComCidadeEmBranco - Se deve gerar com matriculas que tenham a cidade em branco e sem caixa postal
   * @param array $aParcelasObrigatorias - Verifica se a matricula possui as parcelas obrigatórias passadas para geração
   * @param integer $iMaximoParcelasParaGerar - O Máximo de parcelas para poder gerar, incluiindo a parcela unica
   * @param string $listamatrics - Lista de matriculas para geração
   * @param string $sParcelaUnica - Lista de parcelas unicas utilizadas na geração
   * @param integer $anousu - Ano para o qual deseja realizar a emissão
   * @param integer $quantidade - Quantidade de matriculas na query
   * @param integer $iAnosConsiderarMovimento - Quantidade de anos para considerar na validação de movimentos de anos anteriores
   * @param string $ordem - Ordem que deve ser gerado os recibos
   * @param string $especie - Tipos de imóveis
   * @param string $imobiliaria - Vínculo com a imobiliária
   * @param string $loteamento - Vínculo com loteamentos
  ...
  public static function emitir(
    ...
    /**
     * Define os valores padrão dos parametros
     */
    ...
    /**
     * Busca a taxa bancária e as informações do tipo de débito
     */
    ...
    /**
     * Verifica se o convênio é um convenio de cobrança válido
     */
    $lConvenioCobrancaValido = CobrancaRegistrada::validaConvenioCobranca($oRegraEmissao->getConvenio());

    /**
     * Cria uma nova emissao geral para essa geração
     */
    ...
    for ($i = 0; $i < $quantidade; $i++) {
      ...
      /**
       * Faz a chamada do callback para atualizar o andamento da geração dor recibos
       */
      $callbackAtualiza($i, $quantidade);

      /**
       * Começa a executar as validações com base nos filtros passados
       * Também busca os dados para emissão dos recibos
       */
      ...
      /**
       * Verifica se deve ou não gerar o recibo quando não tem cidade e caixa postal com base no parametro
       */
      ...
      /**
       * Busca as informações do proprietário da matricula
       */
      ...
      if ($lConvenioCobrancaValido) {
        ...
        /**
         * Verifica se o CPF ou CNPJ é valido caso seja um convenio de cobrança valido
         */
      ...
      /**
       * Verifica se o setor e quadra estão na tabela para não ser processado
       */
      ...
      /**
       * Verifica se o CGM esta na tabela para não ser processado
       */
      ...
      /**
       * Verifica se a matricula esta na tabela para não ser processada
       */
      ...
      /**
       * Verifica se o endereço de entrega é valido com base no parametro passado
       */
      ...
      /**
       * Considera as movimentações da quantidade de anos passada por parametro para a matrícula
       */
      ...
      /**
       * Verifica se as parcelas obrigatórias passadas estão em aberto
       */
      ...
      /**
       * Busca o valor total do iptu e a parcela máxima
       */
      ...
      /**
       * Busca as informações do arrecad para o numpre
       */
      $sSqlArrecadExec = "EXECUTE sSqlArrecad({$j20_numpre})";
      $resultfinarrecad = \db_query($sSqlArrecadExec) or die($sSqlArrecadExec);

      if (!$resultfinarrecad) {
        throw new \Exception("Erro ao buscar informações dos débitos para a Matrícula: {$j23_matric}, Numpre: {$j20_numpre}");
      }
      ...
      for ($iParcela = 0; $iParcela < pg_num_rows($resultfinarrecad); $iParcela++) {
        ...
        if (!$rsCalcula  || pg_num_rows($rsCalcula) == 0) {
          throw new \Exception("Erro ao corrigir o valor da parcela {$oDadosParcela->k00_numpar} da Matrícula: {$j23_matric}");
        }
