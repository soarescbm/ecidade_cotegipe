...
foreach ($oDadosDebitos as $aDados) {
  ...
  if ($aDados->k00_numpre != $sHashNumpre ) {
     ...
     if (in_array($oDadosArreTipo->k03_tipo, $aCadTipoBuscaDadosInicialForo) ) {
       ...
       if (pg_num_rows($rsProcessoForo) > 0) {
         ...
         if ( $oParametrosJuridico->v19_partilha == "t") {
            ...
            if (pg_num_rows($rsDadosPartilha) > 0) {
              ...
              try {
                 $nTotalDebito = $oDaoProcessoForoPartilhaCusta->getCustasProcesso('',$iProcessoForo,$dtVencimento,$oDadosArreTipo->k03_tipo);
  	...
  	$aDadosProcessado->termo = "";
  	if (in_array($oDadosArreTipo->k03_tipo, $aCadTipoBuscaDadosParcelamento) && ( isset($oDadosTermo) && (count($oDadosTermo) > 0) ) ) {
  	...
  	$aDadosProcessado->custas_situacao = "";
  	$aDadosProcessado->custas_total    = "";
  	if (in_array($oDadosArreTipo->k03_tipo, $aCadTipoBuscaDadosInicialForo) && ( isset($oDadosProcessoForo)  && (count($oDadosProcessoForo) > 0) ) ) {
