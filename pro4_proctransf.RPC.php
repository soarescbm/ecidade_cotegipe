...
switch ($oPost->sMethod) {
	...
	case 'validaDeptoInicial':
		...
		foreach ( $aListaProcesso as $iInd => $iCodProcesso ) {
			...
			$rsAndPadrao      = $clProtProcesso->sql_record($clProtProcesso->sql_query_andpadrao($iCodProcesso,"*","p53_ordem desc"));
	...
	case 'incluirTramite':
		...
		if ( !$lErro ) {
			...
			foreach ($aObjProcessos as $iIndObj => $oProcesso ) {
			  ...
			  $sSqlProcTransferProc   = $clProcTransferProcValida->sql_query_file(null, $oProcesso->iCodProc, '*', null, null);
			  ...
			  if ($clProcTransferProcValida->numrows == 0) {
				...
				if ( $oPost->iGrupo == 2 ) {
					 ...
					 */
					$rsAndPadrao      = $clProtProcesso->sql_record($clProtProcesso->sql_query_andpadrao($oProcesso->iCodProc,"*","p53_ordem"));
					$iLinhasAndPadrao = $clProtProcesso->numrows;
						...
						if ( $oPost->iCodDeptoRec != $oAndPadrao->p53_coddepto && $oAndPadrao->p53_ordem == 1 ) {
							...
							if ( $oProcesso->lSegue )  {
								...
								while ( $lFeriado ) {
									...
									$rsConsultaFeriado = $clCalend->sql_record($clCalend->sql_query_file(date('Y-m-d',$dtDataFim)));
	...
	case 'buscaProcessos':
		...
		for ($iIndiceProcessos = 0; $iIndiceProcessos < $oDaoProtProcesso->numrows; $iIndiceProcessos++) {
			...
			if ( isset($oProcessoEncontrado->limite) && trim($oProcessoEncontrado->limite) == "") {
			  $lLimite = "false";
			} else if($oProcessoEncontrado->limite <= date("Y-m-d",db_getsession("DB_datausu"))) {
			  $lLimite = "true";
			}
