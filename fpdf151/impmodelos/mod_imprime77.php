...
for ($ii = 0; $ii < $this->linhasdositens ; $ii++) {
  ...
  if ($this->sImprimeDadosDotacao == "t") {
    ...
    for ($j=0; $j < $this->linhasdasdotac; $j++) {

      db_fieldsmemory($this->recorddasdotac, $j);
      $danousu    = pg_result($this->recorddasdotac, $j, $this->danousu);
      $dotacao    = pg_result($this->recorddasdotac, $j, $this->dcoddot);
      $estrutu    = pg_result($this->recorddasdotac, $j, $this->delemento);
      $descrunid  = pg_result($this->recorddasdotac, $j, $this->descrunid);
      $dcprojativ = pg_result($this->recorddasdotac, $j, $this->dcprojativ);
      $dctiporec  = pg_result($this->recorddasdotac, $j, $this->dctiporec);
      $dprojativ  = pg_result($this->recorddasdotac, $j, $this->dprojativ);
      $dtiporec   = pg_result($this->recorddasdotac, $j, $this->dtiporec);
      $ddescrest  = pg_result($this->recorddasdotac, $j, $this->ddescrest);
      $dquant     = pg_result($this->recorddasdotac, $j, $this->dquant);
      $dvalor     = pg_result($this->recorddasdotac, $j, $this->dvalor);
      $dvalor     = db_formatar($dvalor,'f');
      ...
      if (!empty($dotacao)) {
        ...
        if (!in_array($dotacao.$danousu, $arr_antigadotac)) {
          ...
          $this->objpdf->Row(array ('', $dquant, '', "Dotação: ".$dotacao."/".$danousu.$estrutu, $valoritem, $dvalor), 3, false, 3);
...
//    echo $ii." => ".$this->objpdf->gety()." => ".var_dump($muda_pag)."<br>";
  ...
  if(isset($lTemDotacoes) && !$lTemDotacoes) {
